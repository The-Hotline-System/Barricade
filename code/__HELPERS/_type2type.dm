...
/proc/cover_flags2body_zones(bpc, exact_only = FALSE)
	...
	if(bpc == FULL_BODY)
		return list(BODY_ZONE_L_ARM,BODY_ZONE_R_ARM,BODY_ZONE_HEAD,BODY_ZONE_CHEST,BODY_ZONE_L_LEG,BODY_ZONE_R_LEG)
...
/proc/slot2body_zone(slot)
	switch(slot)
		if(ITEM_SLOT_BACK, ITEM_SLOT_OCLOTHING, ITEM_SLOT_SHIRT, ITEM_SLOT_PANTS, ITEM_SLOT_BELT, ITEM_SLOT_ID)
...
/proc/body_zone2item_slots(zone)
	switch(zone)
		...
		if(BODY_ZONE_CHEST)
			return ITEM_SLOT_SHIRT|ITEM_SLOT_OCLOTHING

		if(BODY_ZONE_R_ARM, BODY_ZONE_L_ARM)
			return ITEM_SLOT_SHIRT|ITEM_SLOT_OCLOTHING|ITEM_SLOT_GLOVES

		if(BODY_ZONE_PRECISE_R_HAND, BODY_ZONE_PRECISE_L_HAND)
			return ITEM_SLOT_GLOVES

		if(BODY_ZONE_R_LEG, BODY_ZONE_L_LEG)
			return ITEM_SLOT_PANTS|ITEM_SLOT_OCLOTHING|ITEM_SLOT_FEET
