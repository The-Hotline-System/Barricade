...
window.onerror = function(msg, url, line, col, error) {
    if (document.location.href.indexOf("proc=debug") <= 0) {
        ...
        const debugLine = 'Error: ' + msg + ' | url: ' + url + ' | line: ' + line + extra;
...
let $messages, $subOptions, $subAudio, $selectedSub, $contextMenu, $filterMessages, $last_message;
...
function linkify(parent, insertBefore, text) {
    let start = 0;
    let match;
    const regex = /(?:(?:https?:\/\/)|(?:www\.))(?:[^ ]*?\.[^ ]*?)+[-A-Za-z0-9+&@#\/%?=~_|$!:,.;()]+/ig;
...
function output(message, flag) {
    ...
    if (opts.hasOwnProperty('showMessagesFilters') && !opts.showMessagesFilters['All'].show) {
        ...
        if (opts.hasOwnProperty('filterHideAll') && opts.filterHideAll) {
            let internal = false;
            const messageClasses = messageEl && messageEl.className ? messageEl.className.split(/\s+/) : [];
            ...
            const parentClasses = messageEl.className ? messageEl.className.split(/\s+/) : [];
            const childClasses = messageEl.firstChild && messageEl.firstChild.className ? messageEl.firstChild.className.split(/\s+/) : [];
            ...
            for (let i = 0; i < messageClasses.length; i++) {
                ...
                for (const key in opts.showMessagesFilters) {
                    if (key !== 'All' && opts.showMessagesFilters[key].show === false && opts.showMessagesFilters[key].match) {
                        for (let j = 0; j < opts.showMessagesFilters[key].match.length; j++) {
                            if (opts.showMessagesFilters[key].match[j] === thisClass) {
                                filteredOut = key;
                                break;
                            }
                        }
            ...
            if (!filteredOut && messageClasses.length === 0 && !opts.showMessagesFilters['Misc'].show) {
                filteredOut = 'Misc';
            }
        }
    }
    ...
    if (!filteredOut) {
        ...
        if (bodyHeight + scrollPos >= messagesHeight - opts.scrollSnapTolerance) {
            ...
            if (newMsgEl) {
                ...
                $messages.insertAdjacentHTML('afterend', '<a href="#" id="newMessages"><span class="number">1</span> new <span class="messageWord">message</span> <i class="icon-double-angle-down"></i></a>');
            }
        }
    }
    ...
    if (opts.messageCombining) {
        ...
        if (lastMessage && $last_message && $last_message === trimmed_message) {
            ...
            lastMessage.querySelectorAll('[replaceRegex]').forEach(replaceRegex);
    ...
    if (data === 'softPang') {
        ...
        if (data.clientData) {
            if (opts.restarting) {
                ...
                document.querySelectorAll('.connectionClosed.restarting:not(.restored)').forEach(el => {
                    el.classList.add('restored');
                    el.textContent = 'The round restarted and you successfully reconnected!';
                });
            }
            ...
            if (typeof data.adminMusic === 'string') {
                ...
                if (bindLoadedData) {
                    audioEl.addEventListener('loadeddata', adminMusicLoadedData, { once: true });
                }
            ...
            for (const i in data.syncRegex) {
                ...
                replaceRegexes[i] = [new RegExp(regexData[0], regexData[1]), regexData[2]];
            }
        }
    }
}
...
function createPopup(contents, width) {
    opts.popups++;
    const popupHtml = '<div class="popup" id="popup' + opts.popups + '" style="width: ' + width + 'px;">' + contents + ' <a href="#" class="close"><i class="icon-remove"></i></a></div>';
...
function adminMusicLoadedData() {
    const audioEl = document.getElementById('adminMusic');
    if (opts.musicStartAt && (audioEl.duration === Infinity || opts.musicStartAt <= audioEl.duration)) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (savedConfig.smusicVolume) {
        ...
        document.getElementById('adminMusic').volume = opts.defaultMusicVolume / 100;
    }
    ...
    document.body.addEventListener('mousedown', function(e) {
        ...
        if (target.tagName === 'A' || target.closest('a') || target.tagName === 'INPUT' || target.tagName === 'TEXTAREA') {
    ...
    document.querySelectorAll('.sub, .toggle').forEach(el => {
        ...
        el.addEventListener('mouseleave', () => opts.suppressSubClose = false);
    });
    ...
    document.getElementById('togglePing')?.addEventListener('click', function() {
    ...
    document.getElementById('saveLog')?.addEventListener('click', function() {
        const date = new Date();
        const fname = 'Chat Log ' +
            date.getFullYear() + '-' +
            String(date.getMonth() + 1).padStart(2, '0') + '-' +
            String(date.getDate()).padStart(2, '0') + ' ' +
            String(date.getHours()).padStart(2, '0') +
            String(date.getMinutes()).padStart(2, '0') +
            String(date.getSeconds()).padStart(2, '0') + '.html';
    ...
    document.getElementById('highlightTerm')?.addEventListener('click', function() {
        if (document.querySelector('.popup .highlightTerm')) return;

        let termInputs = '';
        for (let i = 0; i < opts.highlightLimit; i++) {
            termInputs += '<div><input type="text" name="highlightTermInput' + i + '" id="highlightTermInput' + i + '" maxlength="255" value="' + (opts.highlightTerms[i] || '') + '" /></div>';
        }
        const popupContent = '<div class="head">String Highlighting</div>' +
            ...
            termInputs +
            '<div><input type="text" name="highlightColor" id="highlightColor" style="background-color: ' + (opts.highlightColor || '#FFFF00') + '" value="' + (opts.highlightColor || '#FFFF00') + '" maxlength="7" /></div>' +
    ...
    document.body.addEventListener('submit', function(e) {
        if (e.target.id === 'highlightTermForm') {
            ...
            for (let count = 0; count < opts.highlightLimit; count++) {
                const input = document.getElementById('highlightTermInput' + count);
                if (input) {
                    const term = input.value.trim();
                    opts.highlightTerms[count] = term === '' ? null : term.toLowerCase();
                }
            }

            const colorInput = document.getElementById('highlightColor');
            const color = colorInput.value.trim();
            opts.highlightColor = (color === '' || color.charAt(0) !== '#') ? '#FFFF00' : color;

            const popup = document.getElementById('highlightPopup')?.closest('.popup');
    ...
    document.getElementById('toggleCombine')?.addEventListener('click', function() {
