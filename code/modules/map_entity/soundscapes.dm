...
/proc/get_soundscape(soundscape_name)
	if(!soundscape_name)
		return null
	return GLOB.soundscapes_by_name[lowertext(soundscape_name)]

/proc/get_soundscape_by_type(soundscape_type)
	if(!soundscape_type)
		return null
	return GLOB.soundscapes_by_type[soundscape_type]
...
/obj/effect/map_entity/env_soundscape/proc/activate_for_player(mob/M)
	...
	var/datum/soundscape/scape = get_soundscape(soundscape)
	if(!scape)
		return
...
/obj/effect/map_entity/env_soundscape/proc/stop_for_player(mob/M, stop_random = TRUE)
	...
	if(channels)
		for(var/channel in channels)
			SEND_SOUND(M, sound(null, channel = channel))
...
/obj/effect/map_entity/env_soundscape/proc/start_looping_sounds(mob/M, datum/soundscape/scape)
...
/obj/effect/map_entity/env_soundscape/proc/start_random_sounds(mob/M, datum/soundscape/scape)
...
/obj/effect/map_entity/env_soundscape/proc/play_random_sound(mob/M, list/random_def)
	...
	SEND_SOUND(M, sound(wave, volume = volume * 100))
...
/obj/effect/map_entity/env_soundscape/proc/on_exited(datum/source, atom/movable/AM, direction)
	SIGNAL_HANDLER
...
/obj/effect/map_entity/env_soundscape_trigger/proc/on_exited(datum/source, atom/movable/AM, direction)
	SIGNAL_HANDLER
	if(!isliving(AM))
		return
