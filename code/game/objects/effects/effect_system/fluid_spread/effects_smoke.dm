...
/datum/effect_system/fluid_spread/smoke/freezing/proc/Chilled(turf/open/chilly)
	...
	if(chilly.zone)
		var/datum/gas_mixture/air = chilly.return_air()
		var/datum/gas_mixture/new_gas = air.removeVolume(CELL_VOLUME)
		...
		QDEL_NULL(chilly.active_hotspot)
		if(new_gas.getGroupGas(GAS_PLASMA))
			new_gas.adjustGas(GAS_NITROGEN, new_gas.gas[GAS_PLASMA])
			new_gas.adjustGas(GAS_PLASMA, -new_gas.gas[GAS_PLASMA])

		air.merge(new_gas)
		chilly.zas_update_loc()
