...
/datum/component/phylactery/proc/revive_lich(mob/living/corpse)
	...
	lich.equip_to_slot_or_del(new /obj/item/clothing/under/color/black(lich), ITEM_SLOT_SHIRT)
	...
	if(!QDELETED(corpse))
		...
		if(iscarbon(corpse))
			...
			for(var/obj/item/organ/to_drop as anything in carbon_body.processing_organs)
